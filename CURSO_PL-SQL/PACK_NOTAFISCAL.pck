...
create or replace package body PACK_NOTAFISCAL is
 ...
 FUNCTION RETORNA_CLIENTEVALOR RETURN VARCHAR2 IS
   TYPE R_CLIENTE IS RECORD(DS_CLIENTE NOTAFISCAL_CURSO.DS_CLIENTE%TYPE,
                            VL_CLIENTE NUMBER);
   ...
   CURSOR CUR_CLIENTES IS
     SELECT NOTAFISCAL_CURSO.DS_CLIENTE, SUM(NOTAFISCAL_CURSO.VL_NOTA) VL_CLIENTE
       FROM NOTAFISCAL_CURSO
      GROUP BY NOTAFISCAL_CURSO.DS_CLIENTE;                              
 BEGIN
   FOR I IN CUR_CLIENTES LOOP
   ...
   RETURN V_CLIENTE(2).DS_CLIENTE || ' - ' || V_CLIENTE(2).VL_CLIENTE;
