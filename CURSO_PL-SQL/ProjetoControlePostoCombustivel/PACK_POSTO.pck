...
create or replace package body PACK_POSTO is
 ...
 BEGIN 
   ...
   IF I_CD_COMBUSTIVEL IS NULL THEN 
     O_MENSAGEM := 'O tipo de combustível precisa ser informada';
     RAISE E_GERAL;
   END IF; 
   
   IF I_CD_PROPRIETARIO IS NULL THEN
     O_MENSAGEM := 'O proprietário precisa ser informada';
     RAISE E_GERAL;
   END IF;  
 ...
 BEGIN
   ...
   IF V_COUNT > 0 THEN
     O_MENSAGEM := 'O proprietário nao pode ser excluído, pois existem veículos 
   ...
   EXCEPTION
     WHEN OTHERS THEN
       O_MENSAGEM := 'Erro ao excluír o proprietário ' || I_CD_PROPRIETARIO || ': ' || SQLERRM;
       RAISE E_GERAL;
   END;
 ...
 EXCEPTION
   ...
   WHEN OTHERS THEN
     ROLLBACK;
     O_MENSAGEM := '[EXCLUI_PROPRIETARIO] Erro no procedimento de exclusao do proprietário: ' || SQLERRM;
 END; 
 --------------------------------------------------------------------------------
 --------------------------------------------------------------------------------
 --------------------------------------------------------------------------------
 ...
 BEGIN
   ...
   IF V_COUNT > 0 THEN
     O_MENSAGEM := 'O combustível nao pode ser excluído, pois existem veículos 
     associados a ele';
     RAISE E_GERAL;
   END IF;
   ...
   EXCEPTION
     WHEN OTHERS THEN
       O_MENSAGEM := 'Erro ao excluír o combustível ' || I_CD_COMBUSTIVEL || ': ' || SQLERRM;
       RAISE E_GERAL;
   END;
 ...
 EXCEPTION
   ...
   WHEN OTHERS THEN
     ROLLBACK;
     O_MENSAGEM := '[EXCLUI_COMBUSTIVEL] Erro no procedimento de exclusao do combustível: ' || SQLERRM;
 END;                                                                                                          
end PACK_POSTO;
/
